...
find_cheapest_flight_city_4 <-function(country_origin="Canada", city_origin="Montreal", city_destination="Milan", time_frame=30){
  ...
  destination_locs <- find_location(city_destination, location_types = c("CITY"))
  ...
  if (is.data.frame(destination_locs$node) && nrow(destination_locs$node) > 0) {
    ...
    cat("\n✓ Using", city_destination,"ID:", destination_id, "\n\n")
  } else {
